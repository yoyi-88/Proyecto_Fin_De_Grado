                ...
                <tbody>
                    <?php while ($user = $this->users->fetch()): ?>
                        <tr class="">
                            ...
                            <td>
                                <span
                                    class="badge <?= ($user->role_name == 'Administrador') ? 'bg-danger' : (($user->role_name == 'Editor') ? 'bg-warning text-dark' : 'bg-secondary') ?>">
                                    ...
                                    <?php if ($_SESSION['user_id'] != $user->id): ?>
                                        <form method="POST" action="<?= URL ?>user/delete/<?= $user->id ?>"
                                            ...
                                            <button type="submit" class="btn btn-danger btn-sm" title="Eliminar"
                                                onclick="return confirm('¿Confirmar eliminación del usuario <?= htmlspecialchars($user->name) ?>?')">
                                                <i class="bi bi-trash3"></i>
                                            </button>
                                        </form>
                                    <?php else: ?>
                                        <button class="btn btn-secondary btn-sm" disabled
                                            title="No puedes borrarte a ti mismo"><i class="bi bi-trash3"></i></button>
                                    <?php endif; ?>
